...
function cardHTML(p,i,prefix,toFicha){
  ...
  return `<div class="card ${on?'picked':''} ${p.sold?'sold':''}" id="${prefix}${p.id}" style="animation-delay:${Math.min(i*.02,.4)}s" onclick="${goCard}">
      ...
      <div class="cfoot-card">${_showBrand?`<div class="cbrand">${escHtml(_bl)}</div>`:''}<div class="ctitle-row"><div class="cmodel">${escHtml(_modelTxt)}</div></div>${_priceRow}${conTalla?`<div class="cship">Envío gratis</div>
