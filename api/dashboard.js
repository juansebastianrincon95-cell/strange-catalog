  ...
  try {
    ...
    if (sbSvc) {
      let q = sbSvc.from('orders')
        .select('subtotal,total,created_at,status,pago')
        .eq('status', 'venta');
    ...
    // ── 3. CRUCE: ROAS y CPA (solo si tenemos inversión) ───────────────────
    ...
    return res.json({
      ...
      // Contexto para interpretar (atribución a nivel cuenta, no por campaña):
      nota: 'ROAS y CPA son a nivel de cuenta (facturación total ÷ gasto total de Meta). No es atribución por campaña.',
      meta_error            // null si todo ok; string si Meta no devolvió gasto
    });
  } catch (err) {
